...
model1 <- survival::survreg(data = dataset2,
                  formula = Surv(splength, rcensor) ~ 1+ time, dist = "exponential", 
                  cluster = schoolid) 
...
model2 <- survival::survreg(data = dataset3,
                  formula = Surv(splength, rcensor) ~ 1 + time, 
                  dist = "weibull", 
                  cluster = schoolid) 
...
<!-- https://devinincerti.com/2019/06/18/parametric_survival.html#generalized-gamma-distribution --> 

```{r}
library(stdReg)

parfrailty(formula=splength ~ avgage + marstat + lowgroup, 
           data = dataset2 %>%
             filter(!is.na(splength), !is.na(avgage), !is.na(marstat), !is.na(lowgroup)))
```
...
Compare the estimates of the Cox model (question 4) with the results of the PWC model with unobserved heterogeneity.
